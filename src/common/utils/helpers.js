...
export const getSuggestions = (suggestions, value, propName) => {
...
export const splitSpace = (arrTitle) => {
  let result;
  if (arrTitle) {
...
export const findMatchesItem = (list, value, propName) => {
  let result;
  if (value) {
    let escapedValue = escapeRegexCharacters(value.trim());
...
export const findMatchesHightlight = (text, query) => {
  let result = [];
  ...
  for (let i = 0; i < queries.length; i++) {
    ...
    if (index > -1) { result.push([index, index + c]); }
  }
 ...
 */
export const buildUrlQuery = (parameters) => {
  let queryUrl = '';
  let qs = '';
...
export const getEnumDescription = (object, value) => {
